...
test.describe("QR 화면의 안내 대상", () => {
  /* 관문은 홈보다 앞이라(기획 2026-08-12) 그냥 열면 이것이 첫 화면이다. 다만 안내를
     켜는 자리(프로필 3/3)는 관문 **뒤**라, 오늘의 사용자 경로로는 안내가 켜진 채 이
     화면에 설 수 없다(저장본 a11y 도 홈에서 확인한 뒤에야 적용된다). 여기서 재는 것은
     «안내가 켜졌을 때 이 화면이 무엇을 가리키는가»라는 화면 계약이므로, 안전 중단
     검사(lane-d D12)와 같은 방법으로 클래스를 직접 얹어 그 상태를 만든다 — 각 검사에서
     카메라 국면이 정해진 **뒤에** 얹는다(그 전에 얹으면 상태 변화 렌더가 도로 지운다).
     진행 표시 보류(시안 5걸음)가 QR 을 흐름 안 걸음으로 되돌리면 이 우회도 같이 걷어낸다. */
