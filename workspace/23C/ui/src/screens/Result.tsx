 ...
 * 주문이 확정되면 S15 가 먼저 «오늘 입력한 내용을 저장할까요?»를 묻는다. 여기서는
 * 저장한 사람에게 결과를 사실로 알리고, «이번만 사용»을 고른 사람에게는 이점을 들어
 * 한 번 더 권한다(QA 4차 2026-08-13) — 답은 아래 «이 기기에 저장하기» 버튼이다.
 *
 * **이 화면은 카드를 쌓지 않는다.**
 *
...
export function Result() {
  ...
  return (
    ...
    >
      ...
      {/* 저장 안내 — 저장한 사람에게는 사실을 알리고, 안 한 사람에게는 한 번 더 권한다.
       *
       * S15에서 «이번만 사용»을 고른 사람에게 «지웠습니다»라고만 알리면 문이 닫힌
       * 것처럼 읽힌다(QA 4차 2026-08-13). 아직 늦지 않았으므로 여기서는 이점(다음
       * 방문 시 바로 추천)을 들어 다시 묻고, 화면 아래 «이 기기에 저장하기» 버튼이
       * 그 물음의 답이 된다. 저장을 마친 사람에게는 예전처럼 사실만 알린다. */}
      <h3 className="resfact">{savedSession ? "이 기기에 저장했습니다" : "오늘 입력한 내용을 저장할까요?"}</h3>
      <p className="hint">
        {savedSession
          ? t(
          ...
          : "다음 방문 시 입력 과정 없이 바로 메뉴를 추천받을 수 있어요."}
      </p>
