...
export function MenuConfirm() {
  ...
  /* 뒤로는 질문으로 돌아간다. 추천은 답의 결과이므로 «앞»은 언제나 질문이고,
     조건을 통째로 손보는 길은 아래 «수정하기»에 따로 있다. */
  ...
  return (
    <Screen
      ...
      actions={
        <>
          {/* 순서는 «선택하기»(주황)가 위, «수정하기»가 아래 — 2차 QA(2026-08-13)로
              기획 목업의 순서를 뒤집었다. 주 동작이 위에 서는 다른 화면들과 같아졌다.
              «수정하기»(옛 «다시 추천받기», QA 5차 2026-08-14)는 조건을 고쳐 다시 받는
              화면으로 보낸다 (기획 3번) — 라벨이 목적지(조건 수정)를 그대로 말하게 됐다. */}
