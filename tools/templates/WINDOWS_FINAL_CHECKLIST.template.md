...
> 이 문서는 `{{TEMPLATE_PATH}}` 에서 생성됩니다. 직접 고치지 마세요.
> 참가팀 ZIP 루트와 운영진 `release/` 에 같은 내용이 들어가며 SHA-256 이 일치합니다.

Windows 사용자는 `{{START_BAT}}` 을 실행하기 전에 이 문서를 먼저 확인하세요.
...
- [ ] 받은 파일이 **Participant ZIP** 인지 확인 (`{{ZIP_NAME}}`)
...
- [ ] 경로가 너무 길어 오류가 나면 짧은 경로로 옮기세요
      예: `C:\KioBridge\`
- [ ] 무결성 확인: `certutil -hashfile {{ZIP_NAME}} SHA256`
      출력이 `{{ZIP_NAME}}.sha256` 의 값과 같아야 합니다
...
- [ ] 콘솔 창에서 `Ctrl+C`
- [ ] `{{STOP_BAT}}` 실행
- [ ] {{WEB_PORT}} 포트 종료 확인: `netstat -ano | findstr :{{WEB_PORT}}`
- [ ] {{API_PORT}} 포트 종료 확인: `netstat -ano | findstr :{{API_PORT}}`
...
| 포트 충돌 | `netstat -ano \| findstr :{{WEB_PORT}}` → `taskkill /PID <pid> /F` |
| Windows Defender · SmartScreen 경고 | "추가 정보" → "실행" (배치 파일에 서명이 없어 뜨는 경고입니다) |
| `{{START_BAT}}` 이 즉시 닫힘 | cmd.exe 를 먼저 열고 배치 파일 경로를 입력해 오류 메시지를 확인 |
| 경로 문제 | 짧은 경로(`C:\KioBridge\`)로 옮겨 다시 시도 |
| 권한 문제 | 사용자 폴더 아래로 옮기기. `C:\Program Files\` 아래는 피하세요 |
| 브라우저가 자동으로 열리지 않음 | <http://127.0.0.1:{{WEB_PORT}}> 을 직접 입력 |
| API health 실패 | <http://127.0.0.1:{{API_PORT}}/health> 확인, 콘솔 오류 확인 |
...
제품 버전 {{PRODUCT_VERSION}} · inputContractVersion {{CONTRACT_VERSION}} ·
