...
이 저장소를 **clone** 해서 여기에 브랜치를 직접 올립니다.

```
git clone https://github.com/zxcv718/kiobridge-23c.git
```

포크에서 올린 PR 에는 GitHub 이 저장소 비밀값을 **일부러 주지 않습니다**(모르는 사람이
워크플로를 고쳐 토큰을 가져가는 것을 막으려고). 그래서 포크에서는 자동 병합도 배포도
돌지 않고 검사만 반쪽으로 돕니다. 초대를 받으셨다면 이 저장소에 바로 push 할 수 있습니다.
...
**커밋은 각자 이름으로 남습니다.** `git log` · GitHub Contributors 그래프 · PR 작성자 모두
쓴 사람 그대로입니다. 다만 **병합 커밋 한 줄**과 PR 화면의 "merged this" 는 저장소 주인
이름으로 찍힙니다 — 자동 병합은 «예약을 건 사람을 대신해» GitHub 이 실행하기 때문입니다.
봇 이름으로 병합하면 그 push 가 배포를 깨우지 못해(GitHub 은 자동화가 일으킨 이벤트로
새 워크플로를 돌리지 않습니다) 이렇게 두었습니다.

**main 에 직접 push 할 수 없습니다.** 저장소 주인을 포함해 누구도요. `gate` 를 통과한
커밋만 들어갈 수 있고, 그 통과는 브랜치에서 생깁니다. 급한 한 줄이어도 브랜치를 거칩니다.
